...
用户明确意图优先。完整新产品规划可先推荐 `/pm-brainstorm`，但显式 `/pm-demand` 等局部任务进入对应 skill，自身检查缺失证据。已有相关资料可复用，无需重复提问。用户只讨论、不写文件时仅在聊天中给建议；不创建目录或文档，除非用户后来要求产物。
...
5. 只讨论时不写文件；需要关键输入时先问并等待。
6. 已有相关且有效的资料可复用，无需重复提问；不相关文档不能替代证据。
...
**证据类**: 核对来源、证据审计、数据真假、结论依据 → `/pm-evidence-audit`；PRD 完整性与可行性评审则用 `/pm-prd-review`
...
> 完整新产品流程默认推荐 brainstorm；显式局部任务、相关已有材料或用户要求跳过时可直达目标技能，并标注尚未验证的环节。
...
| 未运行检查就声称已完成 | 先验证，再陈述 |
| 用户明确要求轻量流程 | 尊重范围，标明未验证假设，不伪造已完成环节 |
