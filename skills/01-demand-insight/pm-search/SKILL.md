...
如果调用时携带 `--type=xxx` 参数（如 `/pm-search --type=competitor`），直接映射到对应搜索类型，**跳过步骤 1 的交互选择**：

| 参数 | 搜索类型 | 执行路径 |
|------|---------|---------|
| `--type=market` | 市场调研 | 步骤 2A - 市场调研 |
| `--type=competitor` | 竞品分析 | 步骤 2A - 竞品分析 |
| `--type=data` | 行业数据 | 步骤 2A - 行业数据 |
| `--type=opinion` | 舆情监控 | 步骤 2A - 舆情监控 |
| `--type=legal` | 合规检查 | 步骤 2A - 合规检查 |
| `--type=all` | 综合搜索 | 步骤 2B - 综合并发搜索 |

解析到类型后，直接跳转对应执行路径，无需询问用户。
如果 `--type` 值不在上述映射中，视为未知参数，降级到步骤 1 的交互选择。
**注意**：如果当前对话中用户未明确提及产品/行业领域，应先询问确认后再执行搜索。

如果**未携带** `--type` 参数（用户直接输入 `/pm-search` 或自然语言触发），继续步骤 1 的交互选择。
