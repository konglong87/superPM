...
产出要求：每项排序保留需求标识、证据来源及日期、评分依据和置信度；缺失数据标为假设，不以估算冒充事实。将最终取舍及责任人传递给 MVP 方案。

**缺数据的 RICE 门禁**：如果需求只有 A/B/C 之类占位代号，且缺少 Reach（覆盖人数）或 Effort（工作量）估计，不得替 A/B/C 填任何精确数值或计算示例分数；即使写“假设”“仅示例”也会让占位符看起来已有排序。只给空白输入表、公式、待补的数据和一个关键澄清问题，STOP and WAIT。获得各需求含义和同口径的估计后，才可计算或展示标注假设的区间。
