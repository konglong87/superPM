...
name: pm-demand
description: |
  Use when: 用户明确要求需求调研、痛点验证，或已有产品需要验证一个具体需求
  Do NOT auto-select when: 用户只想发散新产品方向（优先 pm-brainstorm）或只是简短讨论且不需正式调研
  Direct /pm-demand requests may proceed with their own evidence checks; a full new-product chain can recommend brainstorm first.
allowed-tools:
  - Read
  - Write
  - AskUserQuestion
...
先理解本次范围：完整调研报告还是只讨论一个需求？**只讨论时不写文件**，不自动创建 `docs/`。若已有相关且有效的需求报告，读取并复用，**无需重复提问**；仅同名却与当前产品不相关的文件不能复用。

完整新产品链默认建议先 `/pm-brainstorm`。用户显式要求局部需求验证、明确跳过 brainstorm 或已有相关创意材料时，直接验证需求，不强迫走完整链；缺少目标用户或问题定义时只问缺失的关键信息。不要把建议跳过解释为“头脑风暴已完成”。

---

产出要求：区分已核实证据（来源、日期）、待验证假设与用户决策；不要把访谈或市场数据编造成事实。后续优先级与 PRD 应能引用本报告中的需求及证据。
...
5. 只讨论时不写文件；正式产物仅在用户要求并确认目标后生成。
6. 已有相关且有效的 docs 可以复用，无需重复提问；不相关文档不能替代证据。

---

## 执行流程

下述为**完整调研报告模式**的详细步骤。轻量讨论仅执行当前问题所需的环节；信息已提供时直接引用而不是重复询问。任何跳过的验证均标注为待验证，不能伪称完成。

### 步骤 1: 补齐必要的产品基础信息

**关键原则**：
- 只问缺失的关键信息；已提供的信息无需重复提问
- 每个问题清晰、聚焦
- 等待用户回答后再问下一个
...
仅当用户要求联网/市场验证、当前环境支持且有明确可验证的问题时，才考虑并行验证。轻量讨论、禁止联网或资料已足够时跳过本节，并标注未做外部验证；不要为每个需求无条件派发 agent。
...
用户只要讨论时，在聊天中说明结论与未验证假设，跳过写入。正式报告需确认产出路径；已有文件需先确认覆盖/追加。确认后才创建 `docs/01-需求调研/`，使用 Write 工具创建 `docs/01-需求调研/需求调研报告.md`：
...
> ✅ 需求调研完成！
>
> 📄 若实际生成报告：`docs/01-需求调研/需求调研报告.md`；否则只汇报聊天中的结论，不声称文件存在。
>
> 🎯 建议下一步：
>
...
如果用户中途取消，停止并简要说明已确认与未确认内容。**不要自动保存草稿或创建目录**；只有用户明确要求保存时，确认位置与覆盖风险后才写入。
