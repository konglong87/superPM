...
### 步骤 4.5: 瓶颈关联矩阵分析（v2.5新增）

在收集所有 subagent 结果后，使用瓶颈关联矩阵识别跨环节连锁效应，为步骤5的综合分析提供框架。

**瓶颈关联矩阵**：横向展示每个环节的瓶颈如何影响其他环节，★ 表示强影响，× 表示弱影响或无直接影响。

**使用 AskUserQuestion 引导用户确认瓶颈清单**：

> 📊 瓶颈关联矩阵分析
>
> 基于 subagent 分析结果，识别到的瓶颈如下。请确认是否覆盖了主要瓶颈：
>
> A) 激活率低（激活环节瓶颈，影响留存+变现+传播）
> B) 新用户留存差（留存环节瓶颈，影响变现）
> C) 付费门槛高（变现环节瓶颈，影响收入）
> D) 获客渠道质量差（获取环节瓶颈，影响所有下游环节）
> E) 传播机制弱（传播环节瓶颈，影响自增长）
> F) 以上都有，还有补充

**瓶颈关联矩阵（参考模板）**：

| 瓶颈 | 影响 Acquisition | 影响 Activation | 影响 Retention | 影响 Revenue | 影响 Referral |
|------|:---:|:---:|:---:|:---:|:---:|
| 激活率低 | × | ★ | ★ | ★ | ★ |
| 新用户留存差 | × | × | ★ | ★ | × |
| 付费门槛高 | × | × | × | ★ | × |
...
1. **列出瓶颈**：从5个 subagent 结果中提取各环节前2个瓶颈。
2. **评估连锁效应**：每个瓶颈对其他4个环节的影响程度（★/×）。
3. **识别杠杆点**：找到影响最多环节的瓶颈（即矩阵中 ★ 最多的行），作为最高优先级。
4. **输出分析结果**：将瓶颈关联矩阵写入步骤5的综合分析报告。
...
杠杆点排名：
1. 激活率低 — 影响4个环节（Activation/Retention/Revenue/Referral）← 最高优先级
2. 获客渠道质量差 — 影响3个环节（Acquisition/Activation/Retention）
3. 新用户留存差 — 影响2个环节（Retention/Revenue）
4. 付费门槛高 — 影响1个环节（Revenue）
5. 传播机制弱 — 影响1个环节（Referral）

✅ 结论：优先解决"激活率低"问题，可同时改善留存、变现和传播。
```
