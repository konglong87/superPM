...
2. 完整的新产品规划且缺少相关资料时，默认推荐 `/pm-brainstorm`；直接、局部的 `/pm-demand` 等请求进入该技能自身的门禁。相关产物可复用，无需重复提问。 “我已经想清楚了”但首次设计新产品需求且无相关产物，仍先 `pm-brainstorm`，不直接跳到 `pm-clarify` / `pm-feature`；明确跳过或只要独立局部步骤除外。
