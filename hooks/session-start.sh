...
# [EXPERIMENT] 注入方式对比：注释掉旧的全量注入，改为注入压缩版路由卡片
# 旧方式：注入完整 start-super-pm/SKILL.md（463行/17KB）+ 速查卡片
# 新方式：只注入 routing-card.md（~50行/2KB）
# 如需回退：取消下面注释，删除新注入代码即可
...
#
# # Build the full context string (旧方式)
...
# --- 新注入方式（实验） ---
# 只注入压缩版路由卡片，用户触发 /super-pm 或 /start-super-pm 时通过 Skill 工具按需加载完整内容
routing_card_content=$(cat "${PLUGIN_ROOT}/skills/start-super-pm/routing-card.md" 2>/dev/null || true)
routing_card_escaped=$(escape_for_json "$routing_card_content")

# Build the context string (新方式：只注入路由卡片)
session_context="<EXTREMELY_IMPORTANT>\nYou have super-pm - a Product Manager Skills Pack.\n\n**Below is your routing card for PM skills. For detailed instructions, use the 'Skill' tool to load a specific skill (e.g. Skill pm-brainstorm):**\n\n${routing_card_escaped}\n${project_config_escaped}\n${docs_status_escaped}\n</EXTREMELY_IMPORTANT>"
